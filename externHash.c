...
#define TAM 100 //Tamanho de endereçamento para a Hash Externa
...
void  ERetiraMaiusculas(char *Palavra, char *Destino);
...
int EComparaPalavra(char *Palavra1Buscada, char *Palavra2){
  int i;
  if(Palavra1Buscada[0] == 'a' && Palavra1Buscada[1] == 'l' && Palavra1Buscada[2] == 'b'){
    // printf("\nComparando %s COMMMM %s\n", Palavra2, Palavra1Buscada);
  }
...
Nodo EConsultaPalavra(TabelaExtern EHash, char *Palavra, int Op, int* vetor){
  ...
  while(NodoPalavra){                                     //faz um loop até o dim da lista na posição de hash encontrada
    if(//NodoPalavra->Palavra[0] == 'a' && NodoPalavra->Palavra[1] == 'l' && NodoPalavra->Palavra[2] == 'b'&&
       Palavra[0] == 'a' && Palavra[1] == 'l' && Palavra[2] == 'b'){
      // printf("Nodo Palavra: %s com %s\n", NodoPalavra->Palavra, Palavra);
    }
    if(EComparaPalavra(Palavra, NodoPalavra->Palavra)){   //se encontrar ambas as palavras forem iguais ele retorna o endereço do nodo
      return(NodoPalavra);
    }else{
      ...
      for(i=0; i < strlen(frase); i++){                                         //loop em toda a frase lida
        if(!isalpha(frase[i])){                                                 //enquanto o caracter lido for considerado uma letra ele continua lendo, quando encontrar algo diferente ele considera como fim de uma palavra
          if(EVerificaPalavraValida(&frase[PInicio])){                          //verifica se o tamanho da palavra é valido
            ...
            if(linha == 70 && frase[PInicio] == 'a' && frase[PInicio+1] == 'l' && frase[PInicio+2] == 'b'){
              // printf("Palavra: %s", &frase[PInicio]);
              if(NodoPalavra){
